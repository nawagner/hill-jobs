...
export function Subscribe() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNeedsConfirmation(res.message !== "Subscription filters updated.");
    } catch (err) {
    ...
    return (
      <main className="mx-auto max-w-xl px-6 py-20 text-center">
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-8">
          ...
          <h2 className="mt-4 font-display text-xl font-bold text-emerald-800">
            {needsConfirmation ? "Check your email" : successMessage}
          </h2>
          <p className="mt-2 font-body text-sm text-emerald-700">
            {needsConfirmation ? (
              ...
              <>
                Your weekly digest preferences for <strong>{email}</strong> have been updated.
              </>
            )}
          </p>
        </div>
